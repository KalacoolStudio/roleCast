...
每個 plot 可設定 1–3 通，每通語音預設最多 180 秒。背景最多 2,000 字；驗證失敗會明確停止演練並保留已有紀錄。
...
Judge 每秒取得新增逐字片段，逐次檢查累積證據；檢查期間音訊繼續播放，符合停止條件時立即清空後續播放。每通語音累計預設 **180 秒**，靜音及重新開啟語音均不重設時間。內建劇本預設全場最多 3 通；新演練會保存當時的通話與語音秒數上限。
