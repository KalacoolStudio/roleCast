...
本機瀏覽器中的多角色語音演練。選擇內建防詐／面試劇本，或建立自己的 plot，與 Persona 語音對話，結束後查看逐字稿及附有證據引用的回饋報告。
...
`API_KEY` 同時供後端 Agent 與 GPT Live 使用，不需另外設定語音金鑰。語音介面必須設定 `API_KEY`；舊有 `LLM_API_KEY` 僅供既有後端相容，不能啟用演練介面。兩者都有時，以 `API_KEY` 為準。`LLM_BASE_URL` 與 `LLM_MODEL` 繼續設定後端推論。
...
開啟 **http://127.0.0.1:3000**（或設定的 PORT）。預設使用本機模式；可另外設定下述 GCP 多人工作空間。
...
此介面沒有帳號登入；取得公開網址的人會建立自己的隔離工作區，無法讀取其他工作區的劇本或演練紀錄，但仍能啟動會使用共用模型額度的演練。僅在需要分享時開啟 tunnel。工作區憑證保存在瀏覽器 Cookie；清除 Cookie、換瀏覽器或使用無痕視窗會建立新的空白工作區，無法回到原工作區。

## 劇本工作室：Plot 與 Drill

演練頁面右側提供無銀行品牌的 ATM drawer。語音接通後可查看本場模擬餘額並進行匯款或提款；成功操作會扣除餘額、保存於該次 drill，並以 ATM 操作訊息加入目前通話，讓 Judge、Recap 與 Reporter 能引用這項行動作為證據。匯款僅保存收款帳號末四碼。
...
1. 選擇 plot 後開始；一次只允許一場進行中的演練。內建「防詐警覺演練」會先播放 Lin 的耳機交易示範，從「您好，耳機還在嗎？配件都有嗎？」開始；按聯絡卡的「申請客服回電」才建立通話 drill。
2. 下一位角色準備好後，置中的深色來電彈出視窗會顯示姓名與角色，原舞台與聊天位置保持不變；按綠色「接聽」，允許麥克風權限後直接說話。按紅色「拒接」會結束整場並整理已有紀錄，不啟動麥克風；尚無對話時報告會明示證據不足。可透過 ×、Escape 或背景遮罩收合視窗，再按狀態區的「查看來電」重新開啟；收合不會接聽或拒接。
3. Persona 與 Judge 同時處理語音內容；逐字稿會自動顯示及保存。通話期間仍可靜音、掛斷或結束整場。
4. 每通結束後整理回顧，可能由同一位或另一位角色來電；也可直接結束整場。
5. 演練結束後會自動開啟歷史報告；可用時間軸回看公開角色活動與逐字稿，或點擊證據文字對照原訊息。列表保留已完成、失敗及中斷的演練。

防詐示範可重播、顯示完整對話及放大訂單圖片；選擇「自行找官方客服確認」可查看情境說明。示範是預先編排的內容，不計入你的逐字稿或評分；後續通話使用所選劇本自己的設定。面試與自訂劇本直接進入通話準備。

每個 plot 可設定 1–3 通，每通語音最長 10 分鐘（600 秒）。背景最多 2,000 字；驗證失敗會明確停止演練並保留已有紀錄。

## Drill 即時舞台

使用原本的 `just dev` 啟動後，開始或開啟一場演練，即可在對話旁看到舞台；不需額外啟動動畫服務。

- 老闆 Mastermind 思考並指派角色；職員 Persona 從入口走入通話區，按「接聽」才開始聊天。
...
- 手機採上下排列；可勾選「減少動畫」，也會遵循系統減少動態效果設定。
...
`OPENAI_BASE_URL` 和 `LIVE_VOICE` 可省略，預設如上。語音模型固定為 `gpt-live-1`，client delegation，`store: false`。`LIVE_VOICE` 是每場第一位 Persona 的聲音；後續新 Persona 依建立順序使用不同內建聲音，同一 Persona 再次上場維持原聲音。Persona 名冊只存在於單次 drill，新 drill 會從空名冊重新規劃。規劃、Judge、角色協助與報告共用 `API_KEY`，仍使用 `LLM_BASE_URL` 和 `LLM_MODEL`。只設定舊有 `LLM_API_KEY` 或語音設定無效時，介面會停用「開始演練」並提示設定 `API_KEY`。顯示可開啟語音代表本機設定有效，不代表已驗證帳號或模型權限。修改後端或 `.env` 後需重新啟動。

1. 按「接聽」。瀏覽器先取得麥克風權限，之後才建立付費模型連線。
2. 開場後直接說話，音訊與逐字稿自動傳送及保存，不需按「送出」。對方說話時也可插話。
3. 「麥克風靜音」暫停收音，對方音訊仍可播放。語音中斷時可在同一通按「重新開啟語音」。
4. 「掛斷本通」及「結束整場演練」隨時可用。重新整理或離開頁面會停止語音，返回時須再次手動開啟，不自動續接。演練頁不提供文字輸入。

Judge 每秒取得新增逐字片段，逐次檢查累積證據；檢查期間音訊繼續播放，符合停止條件時立即清空後續播放。每通語音累計最長 **10 分鐘（600 秒）**，靜音及重新開啟語音均不重設時間。內建劇本預設全場最多 3 通；新演練會保存當時的通話與語音秒數上限。

逐字片段可能不完整或延遲，生成文字不代表使用者已聽完整句。歷史標示辨識／播放不確定性；「檢視評估引用片段」與報告引用可查看固定的原始評估文字。往上捲動閱讀時，新字幕不會強制跳回最下方。

瀏覽器會先套用回音消除、降噪及自動增益，再由 AudioWorklet 的自適應 VAD 過濾穩定底噪。VAD 會保留語音開始前 320 ms 與結束後 480 ms；播放角色聲音時會提高啟動門檻來抑制喇叭回授，但較強的使用者語音仍能插話。瞬間傳輸或播放壅塞會捨棄過時片段並繼續通話，連線失效、格式錯誤或逾時才停止。

語音需要支援 AudioWorklet 的瀏覽器及 localhost（或安全來源）。若權限被拒絕、裝置不可用、音訊暫停或模型連線中斷，介面會說明原因並提供手動重開語音。使用耳機仍能得到最穩定的回音抑制效果。開發代理同時支援 HTTP 與 WebSocket。
...
本機模式的逐字稿、角色與報告保存在本機 SQLite；GCP 模式使用雲端持久磁碟。所有工作區共用同一資料庫，但資料列依瀏覽器工作區隔離。**推論所需對話與背景會送至你設定的外部 LLM API**。這不是離線模型；資料保存方式也受供應商政策影響。語音音訊會送往 OpenAI Live；應用程式保存逐字片段與證據，不保存原始音訊，且預設關閉供應商錄製（`store: false`）。防詐操作是模擬情境，沒有真實轉帳或撥打電話。
...
2. 語音接通及掛斷仍使用同一網址，無來源拒絕；短暫切斷串流後能以備援同步接續，不重複通話。
...
語音自動化涵蓋 24/44.1/48 kHz 合成音訊、自適應 VAD、回音門檻、實際 AudioWorklet、可恢復的壅塞處理、回環 WebSocket／Vite 代理、權限失敗、靜音、同時收放音、停止、Persona 聲線、報告引用及資料庫遷移／復原。這些測試沒有驗證真實麥克風或付費 Live 帳號。

語音人工檢查項目：確認帳號可建立 `gpt-live-1`；用實際麥克風說繁體中文並核對辨識；在對方說話時插話；比較耳機與喇叭回音；觀察開場／回覆延遲、口音與音量；確認失去網路、重開語音及掛斷後裝置指示燈關閉；核對不同 Persona 聲音、部分逐字稿與報告引用。請另記錄裝置、瀏覽器、網路、使用秒數與失敗情形。
