...
開啟 **http://127.0.0.1:3000**（或設定的 PORT）。預設使用本機模式；可另外設定下述 GCP 共用工作空間。
...
透過 Google IAP tunnel 使用雲端版，介面會標示 GCP 儲存及授權使用者共用。它仍是一個共用工作空間，一次只能進行一場演練；沒有個別帳號的資料隔離。部署或重啟會中斷進行中的演練並保留既有紀錄。指南包含存取權限、成本、備份、回滾與還原步驟。
