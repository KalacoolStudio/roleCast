...
Server-side JavaScript ESM wrapper for OpenAI's **Live API** and `gpt-live-1`. It provides primary WebSocket audio, WebRTC SDP exchange, and sideband control. It is an independent workspace package; Role Cast's structured planning and evaluation agents remain separately configured.
...
The integration resamples actual device rates to mono PCM16LE 24 kHz in 40 ms packets. A client-side adaptive VAD learns steady room noise, retains a 320 ms prefix and 480 ms tail, and raises its activation threshold during local playback while permitting strong barge-in. Playback is bounded to three seconds; transient capture, relay, provider, or playback congestion sheds stale media instead of ending the call. A 5-second heartbeat has a 15-second ownership deadline. Voice attempts require explicit activation and never reconnect automatically. Mute withholds microphone frames and consumes the same cumulative per-call 600-second budget.

Each drill owns its Persona roster. The configured `LIVE_VOICE` is used for the first Persona, later newly created Personas receive different built-in voices by roster order, and a reused Persona keeps the same voice within that drill. A new drill starts with an empty roster and no Persona identity or conversation history from a prior drill.
