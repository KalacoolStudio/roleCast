...
**Implementation defaults:** Compute Engine with SQLite and private IAP access for the owner or trusted team; configurable project, region, zone, and resource names. Main's browser workspace isolation supplies separate histories and active exercises within that deployment. The user approved implementation and supplied project `project-783f506d-73dd-47d4-96d`; the region defaults to `asia-east1`. Trusted access identities and production secret values remain operator inputs. Public access or Cloud Run would require a different design.
...
Public sign-up or account management, a PostgreSQL migration, autoscaling, zero-downtime deployment, new voice features, and a custom domain. Browser-scoped histories and concurrent independent exercises are supplied by the workspace changes already integrated into main, rather than by this deployment change.
