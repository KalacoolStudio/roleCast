...
The checked-out application already builds React/Vite assets into `apps/web/dist`, which `apps/server/src/app.js` serves alongside the API. `main.js` binds to loopback. Configuration reads `.env` with process-environment precedence. Storage uses synchronous `better-sqlite3`, WAL, schema version 5 after integrating main's browser workspaces, with a single-active-session constraint per workspace. The engine schedules asynchronous model work after returning HTTP 202 and keeps cancellation/job state in memory. Startup calls recovery for every unfinished session. These observations favor an always-running single application instance.

At initial planning the frontend described all records as local and deployment files were absent. The completed implementation adds runtime-aware disclosures, workflows, a container and infrastructure configuration. Main has consolidated its text, plot, stage and voice requirements under `openspec/specs/`. Local-only requirements remain the baseline for local mode; this change adds cloud-mode requirements and a `local-runtime` disclosure delta. Archived changes remain historical evidence and are not rewritten.
...
**Non-Goals:** A managed PostgreSQL rewrite, distributed background jobs, named-account synchronization, automatic data rollback, direct public VM ingress, or transparent continuation of exercises across restarts. These boundaries preserve the behavior of the existing engine rather than implying a scalable multi-user service.
...
Create a dedicated VPC/subnet with no broad ingress rules. Permit TCP 8080 and SSH 22 only from the IAP TCP forwarding range to this VM. The initial VM has an external IPv4 address for outbound image, Google API, and model-provider access, but its baseline ingress firewall permits IAP; the optional HTTPS gateway adds only its dedicated private subnet on port 8080. This avoids requiring a load balancer or Cloud NAT for the initial small deployment; IPv4 and VM usage still incur charges.

Application users receive a VM-scoped IAP tunnel grant conditioned on destination port 8080, plus only the resource-discovery permissions needed by the documented client command. Deployment administrators receive port-22 tunnel and OS Login administration separately. The browser command is `gcloud compute start-iap-tunnel` with a loopback local port; a public HTTPS gateway is provided separately by `add-public-https-access`, while this tunnel remains available for trusted access. IAP provides the authenticated encrypted transport, while the browser talks to a local forwarded HTTP origin. [IAP TCP forwarding](https://docs.cloud.google.com/iap/docs/using-tcp-forwarding)

Inside Docker, bind the backend to `0.0.0.0:8080`, publish the port on the VM, and retain rejection of non-loopback browser Origin headers. The browser's tunnel URL remains loopback. The firewall and IAP form the access boundary; the Origin check is additional browser protection, not authentication. Main's browser-workspace cookie scopes exercise history and active-session limits independently. Gateway access grants do not replace those workspace boundaries.
...
- The application has shared history and a global active session → grant only trusted users and disclose sharing; public accounts require a separate product change.
